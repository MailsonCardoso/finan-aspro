...
export function ContasReceber() {
  ...
  const filtered = entries?.map((d: any) => {
    ...
    const order: Record<string, number> = { 'Atrasado': 1, 'Pendente': 2, 'Recebido': 3 };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              <th className="text-left p-3 font-medium text-muted-foreground w-1/4">Descrição / Cliente</th>
                ...
                <td className="p-3">
                  ...
                  {row.client_id && (
                    <div className="text-xs mt-1 text-muted-foreground">
                      Cliente: {clients?.find((c: any) => c.id === row.client_id)?.name || 'Desconhecido'}
                    </div>
                  )}
                </td>
                <td className="p-3">
      ...
      <SidePanel open={modalOpen} onOpenChange={setModalOpen} title="Nova Conta a Receber">
        <form onSubmit={(e) => {
          e.preventDefault();
          ...
          <div>
            <label className="block text-sm font-medium text-foreground mb-1">Cliente Vinculado</label>
            <select name="client_id" className="w-full border rounded-lg px-3 py-2 bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/30">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">Conta de Recebimento</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input name="issue_date" type="date" required defaultValue={`${selectedYear}-${selectedMonth}-01`} className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
            </div>
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">Data de Vencimento</label>
              <input name="due_date" type="date" required defaultValue={`${selectedYear}-${selectedMonth}-01`} className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 p-3 bg-primary/5 rounded-lg border border-dashed border-primary/20">
            <input
              type="checkbox"
              name="already_paid"
              id="already_paid_income"
              className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
            />
            <label htmlFor="already_paid_income" className="text-sm font-semibold text-foreground cursor-pointer">
              Marcar como Recebido (Lançamento já realizado)
            </label>
          </div>
