...
export function ContasPagar() {
  ...
  const { data: entries, isLoading } = useQuery({
    queryKey: ["financial-entries", "expense", selectedMonth, selectedYear],
    queryFn: async () => {
      const response = await api.get(`/financial/entries?type=expense&month=${selectedMonth}&year=${selectedYear}`);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-foreground tracking-tight">Contas a Pagar</h2>
          <p className="text-sm text-muted-foreground tracking-tight">Gerencie suas obrigações e pagamentos.</p>
        </div>
